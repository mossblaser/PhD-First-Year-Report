\chapter{Background}
	...
	\section{Hardware Neural Simulators}
		...
		\subsection{Topologies}
			...
			% Various used, often constrained by technology but there is a focus on
			% sub-board level interconnect. Here are some examples.
			...
			\subsubsection{Neurogrid}
				...
				% Chips which contain neuron implementations which can recieve and
				% generate spikes (which are fed in/extracted a row at a time out of a 2D
				% grid). These are combined onto a board using a tree-style topology.
				% Notably the tree also has some horizontal links. Routing is implemented
				% using lookup RAMs to select where spikes should go. Separate network
				% used for configuration.
				
				The Neurogrid architecture, at the highest level, consists of a small
				number of Neurogrid chips  (16 in current prototypes) arranged as leaves
				in a binary-tree where spike events are routed to other chips
				\cite{choudhary12}.
			...
			\subsubsection{BrainScaleS}
				...
				% BrainScaleS viewed from one angle is a wafer with many chips connected
				% via an overlay mask in a multi-lane mesh topology with rotating lanes
				% and sparse switches. Mesh is good for locality and maps well onto the
				% very-very 2D space. Twisty multi-lane simplifies routing decisions
				% somewhat.
				% 
				% Zoom out a little and you have some FPGAs bonded onto the wafer where
				% packets are now translated into UDP/IP and pumped into conventional 10
				% gbit/s switches in what is at the moment a star network. It is projected
				% multiple wafers will be combined at this level. Allows scaling but not
				% as nice as an infinitely growing mesh.
				% 
				% Unfortunately the first level is somewhat hard to use so at the moment
				% a lot is falling back to the FPGAs on the next layer.
				...
				rather than split into separate dies. Each chip on the wafer contains a
				number of Analog Neural-Network Chips (ANCs) which can be programmed to
				model up to 254 neurons (depending on the number of connections to other
				neurons) \cite{schemmel10}. There are two types of interconnect in the
				system named `level 1' (L1) and `level 2' (L2) which are described
				below.
	...
	\section{SpiNNaker}
		...
		\subsection{Routing}
			...
			\subsubsection{Multicast}
				...
				% For example, figures (b) - (d) are all optimal in the total number of
				% `hops' required while (a), (c) and (d) are optimal in the number of
				% routing entries required.
