\chapter{Preliminary Work}
	...
	The first section describes an experiment to conducted determine the way in
	...
	experiment to test a simple extension to the SpiNNaker interconnect. The
	proposed topology uses semi-random links to improve its performance while
	...
	\section{SpiNNaker Interconnect Modelling}
		...
		influence the network's performance as they determine the costs involved in
		...
		\subsection{Results}
			...
			\subsubsection{Packet Latency}
				...
				This step change can be clearly seen in the exaggerated links but is
	...
	\section{Wiring-Up Large SpiNNaker Machines}
		...
		\subsection{Reducing Wiring Length}
			...
			length is reduced which can be seen in figure
			\ref{fig:boardsFoldedInterleaved}. While the wiring in this image appears
			more complex, if only on direction (colour) is considered at once, a
			certain amount of regularity can be seen.
		...
		\subsection{Wiring Complexity}
			...
			exists in each group of wires (consider each colour separately in figure
			...
			Based on the above observations, the connections for all 3,600 wires can
			be described using only 53 instructions rather than 3,200. This satisfies the final
			constraint on wiring complexity.
	...
	\section{Small-World Super Computers}
		...
		two people, chosen at random, there is a chain of 6 acquaintances which
		connects them.
		...
		\subsection{Experiments}
			...
			are no-longer logically adjacent but instead is at opposite ends of the
